...
public class PromotionAction extends BaseAction<Promotion> {
    ...
    public String pageQuery() {
        ...
        PageBean<Promotion> pageBean = WebClient
                .create(Constants.BOS_MANAGEMENT_URL + "/bos_management/services/promotionService/pageQuery?page="
                        + page + "&rows=" + rows)
    ...
    public String showDetail() throws IOException, TemplateException {
        //判断id所对应的html是否存在,如果存在直接返回你如果不存在,创建html
        String htmlRealPath = ServletActionContext.getServletContext().getRealPath("/freemarket");
        File htmlFile = new File(htmlRealPath + "/" + model.getId() + ".html");
        //如果html页面不存在,查询数据结合freemarket模板生成生成页面
        if (!htmlFile.exists()) {
            ...
            //获取模板对象
            Template template = configuration.getTemplate("promotion_detail.ftl");//"/bos_management/services/promotionService/promotion/"
            //动态数据
